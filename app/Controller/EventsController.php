...
class EventsController extends AppController
{
    ...
    /**
     * Attach collections where each event is included as an element.
     *
     * @param array $events
     * @return array
     */
    private function __attachCollectionsToEvents(array $events)
    {
        $eventUuids = array_values(array_unique(array_filter(array_map(function ($event) {
            return $event['Event']['uuid'] ?? null;
        }, $events))));
        if (empty($eventUuids)) {
            return $events;
        }
        ...
        $collectionElements = $this->CollectionElement->find('all', [
            'recursive' => -1,
            'conditions' => [
                'CollectionElement.element_type' => 'Event',
                'CollectionElement.element_uuid' => $eventUuids,
            ],
            'fields' => ['CollectionElement.collection_id', 'CollectionElement.element_uuid'],
        ]);
        if (empty($collectionElements)) {
            foreach ($events as $k => $event) {
                $events[$k]['Event']['CollectionMemberships'] = [];
            }
            return $events;
        }

        $collectionIds = array_values(array_unique(array_map(function ($element) {
            return (int)$element['CollectionElement']['collection_id'];
        }, $collectionElements)));

        $this->loadModel('Collection');
        $conditions = ['Collection.id' => $collectionIds];
        if (!$this->_isSiteAdmin()) {
            $conditions['AND'][] = $this->Collection->buildConditions($this->Auth->user('id'));
        }

        $collections = $this->Collection->find('all', [
            'recursive' => -1,
            'conditions' => $conditions,
            'fields' => ['Collection.id', 'Collection.name', 'Collection.uuid', 'Collection.type', 'Collection.description'],
        ]);
        $collectionsById = [];
        foreach ($collections as $collection) {
            $collectionsById[(int)$collection['Collection']['id']] = $collection['Collection'];
        }

        $collectionsByEventUuid = [];
        foreach ($collectionElements as $collectionElement) {
            $collectionId = (int)$collectionElement['CollectionElement']['collection_id'];
            if (!isset($collectionsById[$collectionId])) {
                continue;
            }
            $eventUuid = $collectionElement['CollectionElement']['element_uuid'];
            $collectionsByEventUuid[$eventUuid][$collectionId] = $collectionsById[$collectionId];
        }

        foreach ($events as $k => $event) {
            $eventUuid = $event['Event']['uuid'] ?? null;
            $events[$k]['Event']['CollectionMemberships'] = isset($collectionsByEventUuid[$eventUuid]) ? array_values($collectionsByEventUuid[$eventUuid]) : [];
        }
